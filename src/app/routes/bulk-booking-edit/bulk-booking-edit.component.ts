...
@Component({
  selector: 'app-bulk-booking-edit',
  templateUrl: './bulk-booking-edit.component.html',
  styleUrls: ['./bulk-booking-edit.component.scss'],
  providers: [
...
export class BulkBookingEditComponent implements OnInit {
  start_date: string;
  ...
  displayedColumns: string[] = ['position','s_no', 'p_name', 'qty','time', 'action'];
...
deliveryType = ['DD', 'PP'];
  ...
  ngOnInit() {
    ...
    this.getBookingDetails(this.order_idd)
  ...
  getBookingDetails(orderid) {
    ...
    var getBookingDetailsURL = this.apiservice.getBookingDetailsURL+"?order_id="+orderid+"&shop_id="+this.apiservice.shop_id
  ...
  AssigntoAll(data)
  {
    ...
    this.balance_amount.setValue(data['balance'])

    // console.log(new Date(this.apiservice.changedate(data['booking_date'])))

    this.bookingdate.setValue(new Date(this.apiservice.changedate(data['booking_date'])))
    ...
    // this.customerForm.get('customer_mobile').disable();
    ...
    this.invoiceForm.get("pf_number").setValue(data['booking_no'])
    this.invoiceForm.get("pf_number").disable();

    // this.invoiceForm.get('pf_number').setValue(data['pf_number']);
  ...
  {
    this.booking_date = this.datePipe.transform(dateval.value, 'dd/MM/yyyy');
  }
        ...
        .subscribe(res => {
          ...
          this.form_status = false
            if(res['status'])
            {
              ...
              this.router.navigateByUrl('/bulk-booking');
            }
            else{
  ...
  viewEstimate(title)
  {
      // console.log(event)
      this.form_status = true
      var getEstimateDetailsURL = this.apiservice.getEstimateDetailsURL+"?order_id="+this.order_idd+"&shop_id="+this.apiservice.shop_id
      this.httpClient.get(getEstimateDetailsURL).subscribe(data => {
        ...
        if(data['details'] != null)
        {
          ...
          this.geninvoice.genEstimatePDF(this.data,data['shop_details'],title)
      }
      else{
        this.form_status = false;
        this.toastr.error("No Data Found");
