...
import { MY_DATE_FORMATS_PROVIDER } from 'app/service/my-date-formats';
...
deliveryType = ['DD', 'PP'];
  ...
  getEstimateDetails(orderid) {
    ...
    var getInvoiceDetailsURL = this.apiservice.getInvoiceDetailsURL+"?order_id="+orderid+"&shop_id="+this.apiservice.shop_id
  ...
  AssigntoAll(data)
  {
    ...
    this.total_given = this.advance

    this.invoicedate.setValue(new Date(this.apiservice.changedate(data['invoice_date'])))
    ...
    this.invoiceForm.get("pf_number").setValue(data['invoice_no'])
  ...
  onAdvanceChange(event)
  {
    ...
    if(this.total > 0)
    {
      if(Number(event) > this.total)
      {
        ...
        bal = this.bill_total - (Number(event) + Number(this.total_given))
  ...
  onLabourChange(event)
  {
    ...
    if(this.sub_total > 0)
    {
      ...
      this.round_off = round_off
      bal = this.bill_total - (Number(this.advance_amount.value) + Number(this.total_given))
      this.balance_amount.setValue(parseFloat(String(bal)).toFixed(2))
    }
    else{
      this.toastr.error("Sub Total is 0")
  ...
  calculateInvoiceItems()
  {
    ...
    this.round_off = round_off
    var bal = this.bill_total - (Number(this.advance_amount.value) + Number(this.total_given))
    this.balance_amount.setValue(parseFloat(String(bal)).toFixed(2))
    // this.advance = 0
    // this.advance_amount.setValue('')
  ...
  update_invoice()
  {
    ...
    // console.log(Number(this.advance))
    // console.log(Number(this.total_given))
    if(Number(this.bill_total) < Number(this.total_given))
    {
      this.toastr.error("Bill Amount is Less than Given Amount");
      return
    }
  ...
  {
      this.form_status = true
      var getEstimateDetailsURL = this.apiservice.getInvoiceDetailsURL+"?order_id="+this.order_idd+"&shop_id="+this.apiservice.shop_id
