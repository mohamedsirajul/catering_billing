...
export class ProductComponent implements OnInit {
  ...
  today_date: any;
  displayedColumns: string[] = ['s_no', 'product_name','rate','gst','total_sold','total_bill'];
  printColumns: string[] = ['s_no','product_name','rate','gst','total_sold','total_bill'];
  head = [['S.No', 'Product Name', 'Product Rate','Product GST','Total Sold','Total Bill']];
  ex_head = ['S.No','Product Name', 'Product Rate','Product GST','Total Sold'];
  row_width = ['auto', '*', 'auto','auto', 'auto']
  productValue = "0"
  all_products = []
  ...
  exportpdf() {
    if(this.data.length != 0)
    this.genreport.productReportPDF(this.dataSource.data,this.fulltablename+" - "+this.tablename,this.ex_head,this.row_width,this.start_date,this.end_date)
    // TableUtil.exportPDF(
  ...
  getResults()
  {
    ...
    if(this.start_date != null && this.end_date != null)
    {
      if(start_unix > end_unix)
      {
        ...
        this.apiservice
        .PostToServer('product_report.php', this.parameters)
        .pipe(first())
        .subscribe(data => {
          // console.log(data)
          if(data['data'] != null)
