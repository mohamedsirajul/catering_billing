...
export class EstimatesComponent implements OnInit {
  ...
  displayedColumns: string[] = ['s_no','estimate_no','cus_name','total','paid_status','created_by','delivery_date','action'];
  parameters: {};
  actualdata = {};
  data: any = {};
  notification_id: any;
  ...
  ngOnInit() {
    this.loadEstimateData(this.apiservice.shop_id);
  }

  loadEstimateData(shop_id) {
    this.apiservice.openSpinner()
    this.dataSource.data = []
    this.form_status = true;
    var estimateURL = this.apiservice.estimateURL+"?sid="+shop_id
    this.httpClient.get(estimateURL).subscribe(data => {
      if(data != null)
      {
        console.log(data)
        this.actualdata = data['estimates'];
  ...
  viewEstimate(event,title)
  {
      ...
      this.form_status = true
      this.apiservice.openSpinner()
      var getEstimateDetailsURL = this.apiservice.getEstimateDetailsURL+"?order_id="+event.order_id+"&shop_id="+this.apiservice.shop_id
  ...
  editEstimate(event)
  {
    this.router.navigate(['//estimates-edit', { params : event.order_id }]);
    // console.log(event)
  }
  ...
  cloneEstimate(event)
  {
    this.router.navigate(['//estimates-new', { params : event.order_id }]);
    // this.EstimatetoInvoice(event.order_id)
    // console.log(event)
  }

  estimateClicked(order_id,bulk)
  {
    if(bulk == 1)
    {
      this.form_status = true
      this.apiservice.openSpinner()
      var getEstimateDetailsURL = this.apiservice.getEstimateDetailsURL+"?order_id="+order_id+"&shop_id="+this.apiservice.shop_id
      this.httpClient.get(getEstimateDetailsURL).subscribe(data => {
        this.apiservice.closeSpinner()
        // console.log(data)
        if(data['details'] != null)
