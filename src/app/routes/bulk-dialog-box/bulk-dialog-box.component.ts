...
@Component({
  selector: 'app-bulk-dialog-box',
  templateUrl: './bulk-dialog-box.component.html',
  styleUrls: ['./bulk-dialog-box.component.scss']
})
export class BulkDialogBoxComponent implements OnInit {
  ...
  subProductCtrl1 = new FormControl();
  field_values: any[] = [];
  product_name: any;
  parameters: any;
  product_id: any;
  ...
  chipSelected(new_product)
  {
    // console.log(new_product)
    this.field_values.push(new_product)
    this.field_values = Array.from(new Set(this.field_values));
  }
  ...
  filter2(name: string) {
    return this.all_field_values.filter(field => field.toLowerCase().includes(name.toLowerCase()));
  }
