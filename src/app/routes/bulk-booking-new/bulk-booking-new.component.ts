...
@Component({
  selector: 'app-bulk-booking-new',
  templateUrl: './bulk-booking-new.component.html',
  styleUrls: ['./bulk-booking-new.component.scss'],
  providers: [DatePipe],
  animations: [
...
export class BulkBookingNewComponent implements OnInit {
  start_date: string;
  ...
  getPFnumber() {
    ...
    var getBookingNumberURL = this.apiservice.getBookingNumberURL+"?sid="+this.apiservice.shop_id
    this.httpClient.get(getBookingNumberURL).subscribe(data => {
      ...
      if(data['status'])
      {
          ...
          this.bookingdate.setValue(new Date(this.apiservice.changedate(this.today_date)))
          this.booking_date = this.today_date
  ...
  save_booking()
  {
    ...
    {
      this.toastr.error("Choose Booking Date");
      return
    }
    ...
    if(this.dataSource.data.length == 0)
    {
      this.toastr.error("Add Products to Booking");
      return
    }
